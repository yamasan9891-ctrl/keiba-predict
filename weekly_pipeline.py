...
from static_site.generate_site import generate_index, generate_race_page, generate_win5_page, generate_preview_page


def _preview_match_col(columns, *keywords):
    for col in columns:
...
def build_entries_preview(race_id: str, race_meta: dict, shutuba_df) -> Path:
    """
    枠順（正式な馬番）がまだ発表されていないレース向けに、分かっている範囲
    （馬名・騎手・厩舎・予想オッズ等）だけで軽量な「出走予定」プレビューを作る。
    正式な馬番が決まっていないため、暫定の並び順（表示上の連番）で表示し、
    フルの予想・買い目計算（EV等）は行わない（馬番が変わり得るため無意味なので）。
    """
    cols = shutuba_df.columns
    rename = {}
    for target, keywords in [
        ("horse_name", ("馬名",)), ("sex_age", ("性齢",)), ("jockey_name", ("騎手",)),
        ("trainer_name", ("厩舎",)), ("win_odds", ("オッズ",)), ("popularity", ("人気",)),
    ]:
        found = _preview_match_col(cols, *keywords)
    ...
    return generate_preview_page(race_meta, horses)
...
def build_race_page_data(race_id: str, race_meta: dict, stats, is_win5: bool = False, strategy_config: dict = None) -> tuple:
    ...
    }, strengths, {"favorite_candidate": favorite_candidate, "headline_ev": headline_ev, "race_label": race_label, "tan_odds": odds.get("tan", {}), "popularity": popularity}
...
def run_weekly(dry_run: bool = False):
    ...
    win5_popularity = {}      # race_id -> {horse: 人気順}（WIN5対象レースのみ、穴馬判定用）
    ...
    for rid in race_ids:
        ...
        # 枠順（馬番）がまだ確定・発表されていないレース（開催が数日以上先の場合によくある）は、
        # フルの予想・買い目計算はできないが、出走予定馬・騎手・予想オッズだけは
        # 既に分かっていることが多いので、軽量な「出走予定」プレビューだけ作る
        umaban_col = next((c for c in df.columns if "馬番" in str(c).replace("\n", "").replace(" ", "")), None)
        if umaban_col is not None:
            filled_ratio = df[umaban_col].astype(str).str.strip().replace({"": None}).notna().mean()
            if filled_ratio < 0.5:
                print(f"  △ {rid} はまだ枠順未発表のため、出走予定プレビューのみ作成します")
                try:
        ...
        if is_win5:
            ...
            win5_popularity[rid] = race_analytics.get("popularity", {})
